...
pub struct Hit {
    ...
    pub p: Vector3,
    pub n: Vector3,
    pub uv: [f32; 2], // coord para textura en el cubo
...
pub fn hit_plane_y0(ray: Ray) -> Option<Hit> {
    ...
    let p = ray.o + ray.d * t;
    Some(Hit { t, p, n: Vector3::new(0.0, 1.0, 0.0), uv: [p.x, p.z], id: 0 })
}

// Intersección con cubo AABB centrado en c con half-extent he (0.5 para cubo unitario)
pub fn hit_aabb(ray: Ray, c: Vector3, he: f32) -> Option<Hit> {
    ...
    // Determinar normal y UV por la cara alcanzada (epsilon cercano a la cara)
    let eps = 1e-3;
